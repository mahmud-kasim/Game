...
/**
 * BattleShip.java	Tcp Bağğlantısı
 *
 * @author       Mahmud Kasim
 * @version      2016
 */
public class AmiralBatti extends JFrame
{
	...
	private int difficulty = 1;				//Kolay 0, normal 1, İmkansız is 2!
	...
	public AmiralBatti()
	{
		...
		(fileMenu = new JMenu("Dosya")).setMnemonic('D');		// Dosya menusu yaptık

		newServerGame = new JMenuItem("Yeni Server Oyna...",'S');
		newServerGame.addActionListener(itemHandler);
		fileMenu.add(newServerGame);

		newClientGame = new JMenuItem("Yeni Client Oyna...",'C');
		newClientGame.addActionListener(itemHandler);
		fileMenu.add(newClientGame);

		newGame = new JMenuItem("New oyun...",'G');
		newGame.addActionListener(itemHandler);
		fileMenu.add(newGame);

		restart = new JMenuItem("Uygulama Restart",'R');
		restart.addActionListener(itemHandler);
		restart.setEnabled(false);
		fileMenu.add(restart);

		fileMenu.addSeparator();
		about = new JMenuItem("Hakkinda...",'A');
		about.addActionListener(itemHandler);
		fileMenu.add(about);

		quit = new JMenuItem("Cikis",'x');
		quit.addActionListener(itemHandler);
		fileMenu.add(quit);

		(optionMenu = new JMenu("Ayarlar")).setMnemonic('O');	 // KOnsol menü'm

		help = new JMenuItem("Yardim",'H');
		help.addActionListener(itemHandler);
		optionMenu.add(help);

		feel = new JMenuItem("Bak ve Hisset...",'L');
		feel.addActionListener(itemHandler);
		optionMenu.add(feel);

		optionMenu.addSeparator();
		internet = new JMenuItem("Internet Ayarlari...",'I');
		internet.addActionListener(itemHandler);
		optionMenu.add(internet);

		challenge = new JMenu("Zorluk");

		(easy = new JCheckBoxMenuItem("Kolay")).addActionListener(itemHandler);
		(normal = new JCheckBoxMenuItem("Normal")).setSelected(true);
		normal.addActionListener(itemHandler);
		(impossible = new JCheckBoxMenuItem("imkansiz")).addActionListener(itemHandler);
	...
	private class ItemHandler implements ActionListener
	{
		public void actionPerformed( ActionEvent e )
		{
			...
			if (e.getSource()==newGame)
			{
				...
				switch (difficulty)
				{
					case 0:	level = "Kolay";			break;
					case 1:	level = "Normal";			break;
					case 2:	level = "imkansiz";	break;
				}
			...
			if (e.getSource()==about) {
				JOptionPane.showMessageDialog(c,
					"Amiral Batti\n\n" +
					"---:      Mahmud Kasim\n" +
					"Developer:          Mahmud Kasim\n" +
					"Oyun:            Mahmud Kasim\n\n" +
					" 12 - 04 - 2016",
					"Amiral Batti hakkinda", JOptionPane.PLAIN_MESSAGE);
			}
			if (e.getSource()==quit)	{  System.exit(0);	}


			// konsol menüsünü yönetme
			if (e.getSource()==help)
			{
				new HelpBrowser();
			}
			if (e.getSource()==internet)
			{
				try
				{
					gamePort = Integer.parseInt(JOptionPane.showInputDialog(c.getParent(),
						"iletisim icin port giriniz:", "internet Ayarlari",
	...
	{
		new AmiralBatti();		//Amiral battı classına instance yaptık
	}
}
