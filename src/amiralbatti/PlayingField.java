...
class PlayingField extends JPanel implements Serializable
{
	...
	static Image [][] ships = new Image[6][3];	
	...
	public PlayingField(String gameTitle)
	{
		...
		status = new JTextArea("Amiral Battıya Hoşgeldiniz...\n", 4, 0);
		(statusPanel = new JPanel()).setLayout(new BorderLayout());
		statusPanel.setBorder(new TitledBorder("Oyun Statu Raporu"));
		...
		(carrierB = new JButton("Ucak Gemisi", new ImageIcon(ships[CARRIER][1]))).addActionListener(itemHandler);
		(seawolfB = new JButton("Katil Balina", new ImageIcon(ships[SEAWOLF][1]))).addActionListener(itemHandler);
		(battleshipB = new JButton("Savaş Gemisi",new ImageIcon(ships[BATTLESHIP][1]))).addActionListener(itemHandler);
		(submarineB = new JButton("Deniz Altı",new ImageIcon(ships[SUBMARINE][1]))).addActionListener(itemHandler);
		(patrolB = new JButton("Devriye",new ImageIcon(ships[PATROL][1]))).addActionListener(itemHandler);
		...
		buttonPanel.setBorder(new TitledBorder("Secminizi yapiniz..."));
	...
	public int getHit(Point coordinates)
	{
		...
		if ((theArea / 100) % 10 == 0)
		{
			if (ship>0)
			{
				...
				switch (ship)
				{
					case CARRIER:		shipName = "Ucak Gemisi!";	break;
					case BATTLESHIP:	shipName = "Savas Gemisi!";	break;
					case SUBMARINE:		shipName = "Deniz_Alti!";	break;
					case SEAWOLF:		shipName = "Katil Balina!";		break;
					case PATROL:		shipName = "Devriye!";		break;
				}
				addMessage("Tam isabet !!!  " + shipName);
				if (AmiralBatti.soundOn()) Sound.playHit();
			}
			else
			{
				addMessage("Haddi Bee, Butun gemileri kacirdin :( !  ");
				if (AmiralBatti.soundOn()) Sound.splash.play();
			}
			theArea += 100;
		...
		{
			addMessage("Daha once secilen yeri sectin. YAPMA !");
			theArea = -theArea;
		}
		return theArea;
	}
...
class LeftField extends GridArea
{
	...
	public void paintComponent(Graphics g)
	{
		...
		for (int y=0; y<10; y++) for (int x=0; x<10; x++)
		{
			...
			{
				current = area[x][y]/10;
				if ((current/10)%10!=0)		//==1 olarakta yazılabilir o da bi ihtimal
				{
